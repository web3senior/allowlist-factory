 ...
 * @returns
 */
export async function getUser(wallet_addr) {
  let requestOptions = {
    method: 'GET',
    redirect: 'follow',
  }
  const params = new URLSearchParams({ wallet_addr: wallet_addr }).toString()
  const response = await fetch(`${import.meta.env.VITE_API_URL}user/get?${params}`, requestOptions)
 ...
 * @returns
 */
export async function updateConfig(post, wallet_addr) {
  var requestOptions = {
    method: 'POST',
  ...
  const response = await fetch(`${import.meta.env.VITE_API_URL}config/update?wallet_addr=${wallet_addr}`, requestOptions)
  if (!response.ok) throw new Response('Failed to get data', { status: 500 })
  return response.json()
}
